...
use std::{path::Path, ptr::null_mut as NULL, process::{self}, fs::{self}, io};
...
fn main() {
    if let Some(base_dirs) = BaseDirs::new() {
        ...
        if b.exists() {
            ...
            let box_msg_ask: Vec<u16> = "Phát hiện ZaloPC ở ổ C! Bạn có muốn tiếp tục?\0".encode_utf16().collect();
            let box_msg_confirm: Vec<u16> = "Bắt đầu chuyển ZaloPC sang ổ chỉ định!\0".encode_utf16().collect();
            let box_msg_cancel: Vec<u16> = "Đã huỷ việc chuyển dữ liệu!\0".encode_utf16().collect();
            let box_msg_complete: Vec<u16> = "Chuyển ZaloPC sang ổ chỉ định thành công!\0".encode_utf16().collect();
            unsafe {
                let msgbox_id = winuser::MessageBoxW(NULL(), box_msg_ask.as_ptr(), box_title.as_ptr(), winuser::MB_YESNO | winuser::MB_ICONEXCLAMATION);
                if msgbox_id == IDYES {
                    winuser::MessageBoxW(NULL(), box_msg_confirm.as_ptr(), box_title.as_ptr(), winuser::MB_OK | winuser::MB_ICONINFORMATION);
                }
                else {
                    winuser::MessageBoxW(NULL(), box_msg_cancel.as_ptr(), box_title.as_ptr(), winuser::MB_OK | winuser::MB_ICONINFORMATION);
                    process::exit(0);
                }
            }
            ...
            let box_msg_cancel: Vec<u16> = "Đã huỷ việc chuyển đổi! \n Không phát hiện Zalo trên ổ C!\0".encode_utf16().collect();
